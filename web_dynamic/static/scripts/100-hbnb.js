$('document').ready(function () {
  ...
  let statesOrCitiesDict = {};

  $('input[type="checkbox"].states-input').change(function () {
    if ($(this).is(':checked')) {
      statesOrCitiesDict[$(this).attr('data-id')] = $(this).attr('data-name');
    } else {
      delete statesOrCitiesDict[$(this).attr('data-id')];
	}

    $('.locations h4').text(Object.values(statesOrCitiesDict).join(', '));
  });

  $('input[type="checkbox"].cities-input').change(function () {
    if ($(this).is(':checked')) {
      statesOrCitiesDict[$(this).attr('data-id')] = $(this).attr('data-name');
    } else {
      delete statesOrCitiesDict[$(this).attr('data-id')];
  ...
  $('button').click(function () {
    ...
    $.ajax({
      ...
      data: JSON.stringify({
		...
		'states': Object.keys(statesOrCitiesDict)
      }),
      success: function(data) {
        // Iterar sobre los lugares obtenidos y agregarlos a la sección de lugares
        for (const place of data) {
